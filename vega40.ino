...
void loop() {
	...
	if (timeSinceFirstPress > doubleClickThresh) { goToSleep(); }    // double click didn't happen in time, go back to sleep         
	if (buttonState == LOW) { state = 101; }    // button pressed
    }
	...
	if (currentLEDvalue[0] != safetyBrightness) { currentLEDvalue[0]++; }     // fade in to solidBrightness value
	if (currentLEDvalue[1] != safetyBrightness) { currentLEDvalue[1]++; }
	if (currentLEDvalue[2] != safetyBrightness) { currentLEDvalue[2]++; }
	delay(3);

	if (buttonState == LOW) { state = 1020; }   // button pressed, change state
    }
	...
	if (currentLEDvalue[0] != fashionBrightness) { currentLEDvalue[0]--; }     // fade down to solidBrightness value
	if (currentLEDvalue[1] != fashionBrightness) { currentLEDvalue[1]--; }
	if (currentLEDvalue[2] != fashionBrightness) { currentLEDvalue[2]--; }
	delay(3);

	if (buttonState == LOW) { state = 1040; }   // button pressed, change state
    }
	...
	if (buttonState == LOW) { state = 1060; }    // button pressed to turn off
    }
	...
	if (buttonState == LOW) { state = 1062; }    // button pressed to turn off
    }
	...
	if (buttonState == LOW) { state = 1064; }    // button pressed to turn off
    }
	...
	if (buttonState == LOW) { state = 1066; }    // button pressed to turn off
    }
	...
	if (buttonState == LOW) { state = 1080; }    // button pressed to turn off
    }

    // Waiting for button release to go to sleep
    else if (state == 1080) { 
	// linear fading
	if (currentLEDvalue[0] > 0) { currentLEDvalue[0]--; }       
	if (currentLEDvalue[1] > 0) { currentLEDvalue[1]--; }
	if (currentLEDvalue[2] > 0) { currentLEDvalue[2]--; }
	delay(transitionRate);

	int brightnessSum = currentLEDvalue[0] + currentLEDvalue[1] + currentLEDvalue[2];
	if ((brightnessSum == 0) && (buttonState == HIGH)) { goToSleep(); }       // go to sleep when the button's been released and fading is done
    }
